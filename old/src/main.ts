...
import { baseColor, normalMap, roughnessMap, bumpMap } from './textures/MuddyGrass'
...
const floorMaterial = new MeshStandardMaterial({
  map: tx.load(baseColor),
  metalness: 0,
  normalMap: tx.load(normalMap),
  roughnessMap: tx.load(roughnessMap),
  bumpMap: tx.load(bumpMap),
});
...
function cell(x: number, z: number, north: boolean, south: boolean, east: boolean, west: boolean, start: boolean, finish: boolean) {
  ...
  const _floorMaterial = start ? startMaterial : finish ? finishMaterial : floorMaterial

  const floor = new Mesh( floorGeometry, _floorMaterial );
